...
while True:
    ...
    for entity in results:
        ...
        if korean_label in ["우유", "대두", "밀", "계란", "땅콩"]:
            ...
            detected_words.append((word, korean_label, entity["score"]))
    ...
    print("\n상세 탐지 결과:")
    if detected_words:
        for word, allergen, score in detected_words:
            print(f"- '{word}' → {allergen} (확신도: {score:.2f})")
    else:
    ...
    if matched:
        print("위험")
        print(f"이 식품에는 사용자가 조심해야 하는 성분({', '.join(matched)})이 포함되어 있습니다.")
